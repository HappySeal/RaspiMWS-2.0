##Oncelikle ihtiyacımız olan kutuphanelerimizi ekliyoruz
from flask import Flask
from flask import render_template
##Flask kutuphanesini ekletince projemize ek olarak iki dosya ekleniyor(Static , Templates)
##Templates de web de gorunecek olan html kodumuz , Static dosyamız da web sayfamızda kullanacagımız fotograflar ve grafik oluşturmak için kullanacagımız javascript kutuphanelerimiz
import Adafruit_DHT
import Adafruit_BMP.BMP085 as BMP
##DTH11 ve BMP180 sensörlerimizin kutuphanelerini ekliyoruz
import time,sqlite3,dateStr
## Zamanı bulmal için kullanacagımız time , veri tabanı için kullandıgımız sqlite3 , bulundugumuz tarihi bir yazıya donuşturecek kendi yaptıgım dateStr modullerini ekledik
app = Flask(__name__)
app.debug = True
...
grafikSicaklik = []
grafikBasinc=[]
#Grafige dokecegimiz Tarih , Nem , Sıcaklık ve Basinc listelerini tanımladık
def dakika():
    return int(time.strftime("%H"))+60*(int(time.strftime("%M")))
## Anlık dakika degerini tanımlayacak fonksiyonu tanımlıyoruz
Sondk=dakika()
##Ve 1 kere kayıt alıyoruz
Nem,Sicaklik = Adafruit_DHT.read_retry(11,4)
##DHT 11 Nem ve sıcaklık sensorunden gelen verileri çekiyoruz
con = sqlite3.connect("havaVT.db",check_same_thread=False)
##Veritabanımız ile baglantı kuruyoruz
cursor = con.cursor()
##Degişiklik yapmamızı saglayacak imleç(cursor) degişkenimizi tanımlıyoruz
def table():
    cursor.execute("CREATE TABLE IF NOT EXISTS hava (saatTXT TEXT,tarihTXT TEXT,basinc INT,nem INT,sicaklik INT)")
##Tablomuzu ve bunun bolumlerini tanımlıyoruz.
table()
...
##Grafigimizde yer alacak degerleri veri tabanından çekerek degişkenlerimize ekliyoruz
cikisListesi()
#print(grafikTarih,"\n",grafikNem,"\n",grafikSicaklik,"\n",grafikBasinc,"\n")
@app.route("/")
